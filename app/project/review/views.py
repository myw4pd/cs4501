...
def create_UserReview(request):
    ...
    return _success_response(request, {'title_id': ur.pk})


def lookup_userReview(request, title_id):
    if request.method != 'GET':
        return _error_response(request, "must make GET request")

    try:
        ur = models.UserReview.objects.get(pk=title_id)
    except models.UserReview.DoesNotExist:
        return _error_response(request, "that user review is not found")

    return _success_response(request, {'title': ur.title,      \
                                       'user_id': ur.user_id,          \
                                       'text': ur.text,          \
                                       'date': ur.date \
                                       })
